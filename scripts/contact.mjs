...
const { ART, svgToDataUrl } = await import(pathToFileURL(bundlePath).href);

const SCALE = 4; // 24 -> 96
const cards = Object.entries(ART)
  .map(([key, a]) => {
    const url = svgToDataUrl(a.svg);
    const hx = a.hotspotX * SCALE;
    const hy = a.hotspotY * SCALE;
    return `<div class="card">
      <div class="name">${key}</div>
      <div class="roles">${a.roles.join(", ")}</div>
      <div class="big"><img src="${url}" width="96" height="96"/><div class="hot" style="left:${hx}px;top:${hy}px"></div></div>
      <div class="actual"><img src="${url}" width="24" height="24"/><span>24px · hotspot (${a.hotspotX},${a.hotspotY})</span></div>
    </div>`;
  })
  .join("\n");
...
.big{position:relative;width:96px;height:96px;margin:0 auto;border-radius:4px;
...
.actual{display:flex;align-items:center;gap:8px;margin-top:8px;font-size:10px;color:#ccc;
  background:linear-gradient(90deg,#e8e8e8 0 50%,#1a1a1a 50% 100%);padding:4px;border-radius:4px}
.actual span{background:#2a2a2a;padding:1px 5px;border-radius:3px}
</style></head><body>
<h2>Custom Cursors — art contact sheet (red ring = hotspot)</h2>
